...
%         bus(indx,7) = S_mltp * Q0h(curr_load,time) / S_base;
        % ------------------------add solar panels
        bus(indx,6) = (P0h(curr_load,time) - pv_mltp *solar_winter_day(time)) / S_base;
        bus(indx,7) = S_mltp * Q0h(curr_load,time) / S_base;
...
%         end
        % ---------------------PV and EV
%         if (mod(j,2) == 0)
%             bus(indx,6) = S_mltp * (P0h(curr_load,time)- 8*solar_summer_day(time)) / S_base;
%             bus(indx,7) = S_mltp * Q0h(curr_load,time) / S_base;
%         else
%             if (mod(j,6) == 1)
%                 bus(indx,6) = S_mltp * (P0h(curr_load,time) + ev_shape1(time)- 8*solar_summer_day(time)) / S_base;
%                 bus(indx,7) = S_mltp * Q0h(curr_load,time) / S_base;
%             elseif(mod(j,6) == 3)
%                 bus(indx,6) = S_mltp * (P0h(curr_load,time) + ev_shape2(time)- 8*solar_summer_day(time)) / S_base;
%                 bus(indx,7) = S_mltp * Q0h(curr_load,time) / S_base;
%             else
%                 bus(indx,6) = S_mltp * (P0h(curr_load,time) + ev_shape3(time)- 8*solar_summer_day(time)) / S_base;
%                 bus(indx,7) = S_mltp * Q0h(curr_load,time) / S_base;
%             end
%         end
